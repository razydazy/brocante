  ...
  return {
    ...
    get is_playing() {
      if (this.$store['product-grid-audio-player'].audio !== null) {
        return this.$store['product-grid-audio-player'].audio.src.includes(audio_url) && this.$store['product-grid-audio-player'].is_playing;
      } else {
        return false;
      }
    },
    ...
    onFocusOut: function onFocusOut(event) {
      var productGridItem = event.target.closest('[data-product-grid-item]');

      if (productGridItem && !productGridItem.contains(event.relatedTarget)) {
        this.resetHover();
      }
    },
...
window['QuantityControls'] = function () {
  return {
    ...
    resetQuantity: function resetQuantity($event, value, property) {
      var _this2 = this;

      this.shouldSendEvent = false;
      this[property] = value;
      this.$nextTick(function () {
        _this2.shouldSendEvent = true;
      });
    },
...
window['ThemeComponent_GiftCardRecipient'] = function () {
  return {
    ...
    mounted: function mounted() {
      ...
      document.body.addEventListener('label:cart:adderror', function (e) {
        if (e.detail.source === 'product-form' && e.detail.sourceId === _this.$el.closest('form').getAttribute('id')) {
          _this.errorMessage = e.detail.message;
          _this.errors = e.detail.errors;
          _this.enabled = true;
        }
      });
    }
...
var quantity = __webpack_require__(56);
...
var gift_card_recipient = __webpack_require__(57);
...
register('product', {
  onLoad: function onLoad() {
    ...
    var hasGiftCardRecipientForm = Boolean(this.container.querySelector('[data-gift-card-recipient-form]'));
    ...
    if (this.enableImageZoom) {
      Promise.all(/* import() | product-image-zoom */[__webpack_require__.e(8), __webpack_require__.e(4)]).then(__webpack_require__.bind(null, 70)).then(function (_ref) {
      ...
      return {
        ...
        get currentVariantLocations() {
          ...
          if (storeData && storeData.list) {
            ...
            for (var i = 0; i < storeData.list.length; i++) {
              if (!locationStock[storeData.list[i].name]) {
                if (this.locationsToShow === 'in_stock' && !storeData.list[i].available) {
                  continue;
                }

                locationStock[storeData.list[i].name] = storeData.list[i].available;
              }
            }
        ...
        mounted: function mounted() {
          ...
          promise_stylesheet.promiseStylesheet().then(function () {
            if (_this2.$refs.productPageSplide) {
              _this2.setupSplide();
            }

            _this2.getAddToCartButtonHeight();
          });
          ...
          if (this.$refs.locationInventoryData && this.$refs.locationInventoryData.dataset.locationsToShow) {
            this.locationsToShow = this.$refs.locationInventoryData.dataset.locationsToShow;
          }
          ...
          */
          ...
          var iframeSelectors = '.product-rte iframe[src*="youtube.com/embed"],' + '.product-rte iframe[src*="player.vimeo"]';
        ...
        getAddToCartButtonHeight: function getAddToCartButtonHeight() {
          ...
          if (addToCartBtn) {
            var height = addToCartBtn.offsetHeight;
        ...
        getOptions: function getOptions() {
          ...
          selectors.forEach(function (selector) {
            if (selector.nodeName === 'SELECT') {
              ...
              _this3.options.push(value);

              _this3.optionHandles.push(selector.options[selector.selectedIndex].dataset.handle);
            } else {
              if (selector.checked) {
                var _value = selector.value;

                _this3.options.push(_value);

                _this3.optionHandles.push(selector.dataset.handle);
              }
            }
          });
        },
        ...
        submitForm: function submitForm(evt) {
          ...
          Object(theme_cart["b" /* addItemFromForm */])(this.$refs.productForm).then(function () {
            ...
            _this6.addedToCart = true;
            document.body.dispatchEvent(new CustomEvent('label:modalcart:afteradditem'));

            if (!document.querySelector('[data-show-on-add="true"]')) {
              _this6.$nextTick(function () {
                return _this6.$refs.added.focus();
              });
            ...
            error.json().then(function (errorData) {
              _this6.loading = false;
              var errors = errorData.errors || errorData.description;
              var message = errorData.description || errorData.message; // Gift card recipient form errors are handled in gift-card-recipient.js

              if (!hasGiftCardRecipientForm) {
                _this6.addErrorMessage = message || window.theme.strings.cartError;
              }

              if (errorData.status) {
                document.body.dispatchEvent(new CustomEvent('label:cart:adderror', {
                  detail: {
                    source: 'product-form',
                    sourceId: _this6.$refs.productForm.getAttribute('id'),
                    errors: errors,
                    message: message
                  }
                }));
...
var theme_iframeSelectors = '.rte:not(.product-rte) iframe[src*="youtube.com/embed"],' + '.rte:not(.product-rte) iframe[src*="player.vimeo"]';
Object(utilities["e" /* wrapIframe */])({
  iframes: document.querySelectorAll(theme_iframeSelectors),
  iframeWrapperClass: 'video-wrapper'
